...
# checks if time is within 11 - 10 or 11 to 22
                    ...
                    if 11 <= hours <= 22 and 0 <= minutes < 60:
                        return time_input
        print("\nInvalid time or format. Please enter enter time in HH:MM format, between 11:00 and 22:00.")
...
def get_longest_name (theRosta):
    ...
    print ("\nThe longest customer name of the shift is",currentChamp,"\nWith names of the same lengh being: ",otherChamps)
    
            
def number_ten(hitlist):
    if len(hitlist) >= 10:
        numberTen = hitlist[9]
        theOne = numberTen.name
        print ("\nThe tenth customer of the night was",theOne)
    else: 
        print ("\nThere were nine or less customers")
